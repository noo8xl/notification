...
func GetMONGOdatabaseConfig() [2]string {
	...
	// tmp := getConfigVar("MONGO_DB_LINK")
	// user := strings.ReplaceAll(tmp, "<userName>", getConfigVar("MONGO_DB_USER"))
	// pwd := strings.ReplaceAll(user, "<userPassword>", getConfigVar("MONGO_DB_PASSWORD"))
	// connStr := strings.ReplaceAll(pwd, "<dbName>", getConfigVar("MONGO_DB_NAME"))
